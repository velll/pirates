...
class Grid {
  mapXOffset = 0.03; // padding from left
  mapYOffset = 0.07; // padding from top
  mapXwidth  = 0.85; // width of active portion of the map
  colorCode = {sea: "rgb(0,102,0,0.1)", rock: "rgba(255,73,73,0.5)", port: "rgba(1,1,1,0.5)"};
  ...
  getColor(coords: Coordinates): string {
    let feature: string = this.map.getFeatureByCoords(coords);

    return this.colors[feature];
  }

  getCellPosition(coords: Coordinates) : Position {
